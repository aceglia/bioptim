...
class OsimModel:
    def __init__(self, bio_model: str | osim.Model):
        if isinstance(bio_model, str):
            self.model = osim.Model(bio_model)
        else:
    ...
    # @property
    # def path(self) -> str:
    #     return self.model.getAbsolutePath().toString()

    # def copy(self):
    #     return OsimModel(self.path)

    def serialize(self) -> tuple[Callable, dict]:
        return OsimModel, dict(bio_model=self.path)
    ...
    def segment_index(self, name) -> int:
        return [i for i in self.model.getNumBodies() if name == self.model.getBodySet().get(i).getName()][0]
    ...
    @property
    def segments(self) -> list[osim.BodySet]:
    ...
    def name_dof(self) -> tuple[str, ...]:
        return tuple(self.model.getCoordinateSet().get(i).getName() for i in range(self.model.getNumCoordinates()))
    ...
    def muscle_names(self) -> tuple[str, ...]:
        return tuple(self.model.getMuscles().get(i).getName() for i in range(self.model.getMuscles().getSize()))
    ...
    def forward_dynamics(self, q, qdot, tau, external_forces=None, f_contacts=None) -> MX:
        # pour l'instant que pour les muscles
        # set residual forces
        if external_forces:
            raise ValueError("Forward dynamics using opensim is not implemented with external forces yet.")
        if f_contacts:
            raise ValueError("Forward dynamics using opensim is not implemented with contact forces yet.")

        fs = self.model.getMuscles()
        # update model
        self.state.setQ(q)
        self.state.setU(qdot)
        for i in range(fs.getSize()):
            coord = osim.CoordinateActuator.safeDownCast(fs.get(i))
            if coord:
                coord.setOverrideActuation(self.state, tau[i] * tau[i])
    ...
    def muscle_activation_dot(self, muscle_excitations) -> MX:
        ...
        for k in range(self.model.getMuscles().getSize()):
            self.model.getMuscles().get(k).setExcitation(self.state, muscle_excitations[k])
    ...
    def muscle_joint_torque(self, activations, q, qdot) -> MX:
        ...
        self.state.setU(qdot)
        for k in range(self.model.getMuscles().getSize()):
            self.model.getMuscles().get(k).setActivation(self.state, activations[k])
    ...
    def marker_names(self) -> tuple[str, ...]:
        return tuple(self.model.getMarkerSet().get(s).getName() for s in range(self.model.getNumMarkers()))
